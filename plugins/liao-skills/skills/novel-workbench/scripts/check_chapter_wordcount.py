...
def check_all_chapters(directory: str, pattern: str = '第*.md', min_words: int = 3000) -> list:
    ...
    chapter_files = sorted(dir_path.rglob(pattern))
    results = []
...
def print_results(results: list, min_words: int = 3000) -> bool:
    """打印检查结果"""
    if not results:
        print('没有找到章节文件')
        return False
    ...
    if failed > 0:
        ...
        print('   - 扩展人物内心活动')
        print('   - 补充背景故事')
        print('   参考: references/standards/standard-场景扩写与去注水.md')
...
def main() -> int:
    ...
    if sys.argv[1] == '--all':
        ...
        return 0 if print_results(results, min_words) else 1
    else:
        file_path = sys.argv[1]
        min_words = int(sys.argv[2]) if len(sys.argv) > 2 else 3000
        result = check_chapter(file_path, min_words)
        return 0 if print_results([result], min_words) else 1
