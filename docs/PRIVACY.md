...
Liao Skills is a Codex plugin that packages local skill instructions for reliable task execution, frontend development, agent skill security auditing, and Chinese novel creation workflows.
...
The `reliable-task-execution` skill contains instructions only. It adds no scripts, dependencies, network access, telemetry, or data collection. Any project or external-system action remains subject to the user's environment, permissions, and explicit authorization.
